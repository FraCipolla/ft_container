...
namespace ft
{
    ...
    void resize (size_type sz, T c)
	{
		if (sz > this->size())
		{
			size_type tmp = sz - this->size();
			_current_end = _buffer_size.allocate(tmp);
			for (; tmp > 0; tmp--, this->_current_end++)
				_buffer_size.construct(_current_end, c);
			_end_of_buffer = _current_end;
		}
		else if (sz < this->size())
		{
			size_type tmp = this->size() - sz;
			for (; tmp > 0; tmp--, _current_end--)
				_buffer_size.destroy(_current_end);
			pointer tmp_end = _current_end;
			_current_end = _buffer_start;
			for (; _current_end < tmp_end; _current_end++)
				_buffer_size.construct(_current_end, c);
			_end_of_buffer = _current_end;
		}
	}
    ...
    void clear ()
	{
     ...
     // Special mutative operations on vector

    // void splice (iterator, vector<T, Allocator>&);
    // void splice (iterator, vector<T, Allocator>&, iterator);
    // void splice (iterator, vector<T, Allocator>&, iterator,
    //               iterator);
    ...
    // void merge (vector<T, Allocator>&);
    // template <class Compare>
    // void merge (vector<T, Allocator>&, Compare);
