...
public:
    ...
    pair<iterator, bool> insert(const value_type& value){ return tree_.insert(value); }
    iterator insert(iterator hint, const value_type& value) { return tree_.insert(hint, value); }

    template <typename InputIt>
    void insert(InputIt first, InputIt last)
	{ tree_.insert(first, last); }
    void erase(iterator pos) { tree_.erase(const_iterator(pos)); }
    ...
    pair<iterator, iterator> equal_range(const key_type& key) { return tree_.equal_range(key); }
    pair<const_iterator, const_iterator> equal_range(const key_type& key) const { return tree_.equal_range(key); }
